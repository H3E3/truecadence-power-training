...
<div class="log-card">
  <div class="date">计划中</div>
  <div class="title">近期会继续优化的方向</div>
  <div class="text">
    - FTP 估算：增加 20min / 40min / 60min 参考和估算逻辑说明。<br>
    - FIT Workout / GPX：后续会分开适配，避免把结构化训练课和路线导出混在一起。<br>
    - 反馈管理：后续增加反馈处理状态，方便内测用户知道问题是否已记录/处理中。
  </div>
</div>
<div class="log-card">
...
<div class="ai-panel good">
    ...
    <div class="ai-panel-text">下方结果来自当前数据范围{cache_note}。切换页面回来不会重复扣次数；只有点击重新分析才会重新生成并消耗 1 次额度。</div>
</div>
""", unsafe_allow_html=True)
        st.markdown(st.session_state.ai_diagnosis)
        if st.button("🔄 重新分析", key="ai_reanalyze",
                     disabled=ai_over_limit,
                     help="清除当前诊断，下一次点击开始分析会重新扣 1 次" if not ai_over_limit else "本月额度已用完，暂不能重新分析"):
            st.session_state.pop("ai_diagnosis", None)
...
""", unsafe_allow_html=True)
    ...
    def workout_blocks_for_item(item):
        ...
        def pad(blocks, min_tail=300):
            used = sum(sec for sec, _p in blocks)
            tail = max(min_tail, total - used)
            return blocks + [(tail, z2)]
        if kind in ('z2','fatloss','long','tempo'):
            return [(total, .78 if kind == 'tempo' else (.62 if kind == 'fatloss' else z2))]
        if kind == 'recovery':
            return [(total, z1)]
        if kind == 'sweet':
            blocks = [(600,z2)] + [(900,.90),(300,z2)] * 3
            return pad(blocks)
        if kind in ('threshold','climb'):
            blocks = [(600,z2)] + [(480,.97 if kind=='threshold' else .95),(240,z2)] * 4
            return pad(blocks)
        if kind == 'vo2':
            blocks = [(600,z2)] + [(180,1.10),(180,z2)] * 6 + [(15,1.50),(45,z2)] * 8
            return pad(blocks)
        if kind == 'crit':
            blocks = [(600,z2)] + [(240,.95),(180,z2)] * 5 + [(20,1.35),(100,z2)] * 10
            return pad(blocks)
        if kind == 'openers':
            blocks = [(600,z2)] + [(300,.95),(300,z1)] * 3
            used = sum(sec for sec, _p in blocks)
            return blocks + [(max(180,total-used), z1)]
        if kind == 'race':
            return [(max(900,total), z2)]
        return [(total, z2)]
    def zwo_segments_from_blocks(blocks):
        return "\n".join(steady(sec, frac) for sec, frac in blocks)
    ...
    selected_export_labels = st.multiselect(
        ...
        default=["Zwift .ZWO"],
        help="ZWO 适合 Zwift；ERG/MRC 是常见训练台/训练软件课程格式。FIT Workout 与 GPX 路线导出会后续单独适配。",
    ...
    if DEPLOY_MODE == "server":
        ...
        st.caption("服务器内测版：训练文件会按所选格式打包成 ZIP 下载。ZWO 用于 Zwift；ERG/MRC 适合部分训练台与训练软件。")
    else:
        ...
        with col_path:
            ...
            st.caption("页面显示的课表和导出的训练文件使用同一份数据；FIT Workout / GPX 会后续单独适配。")
